...
class Prescription_model extends CI_Model {
    ...
    public function deleteipdPrescription($ipdid) {
        $this->db->where("basic_id", $ipdid)->delete("ipd_prescription_details");
    }
    ...
    public function getPrescriptionByIPD($id, $visitid = '') {

        $query = $this->db->select('ipd_prescription_details.*,medicine_category.medicine_category,staff.id as staff_id,medicine_dosage.dosage, medicine_dosage.id as dosageid')
                ->join("ipd_prescription_basic", "ipd_prescription_basic.id = ipd_prescription_details.basic_id")
                ->join("ipd_details", "ipd_prescription_basic.ipd_id = ipd_details.id")
                ->join("medicine_category", "ipd_prescription_details.medicine_category_id = medicine_category.id")
                ->join("patients", "patients.id = ipd_details.patient_id")
                ->join("staff", "staff.id = ipd_details.cons_doctor")
                ->join("medicine_dosage", "medicine_dosage.id=ipd_prescription_details.dosage")
                ->where("ipd_prescription_basic.id", $id)
