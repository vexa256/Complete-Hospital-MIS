...
class Mailgateway {
    ...
    function sentRegisterMail($id, $send_to, $opd_no) {

        if (!empty($this->_CI->mail_config) && $send_to != "") {
            $subject = "Patient Registration";
            $msg = $this->getPatientRegistrationContent($id, $opd_no);
    ...
    public function getPatientRegistrationContent($id, $opd_no) {
        $patient = $this->_CI->patient_model->patientProfile($id);
        if ($patient["patient_type"] == 'Outpatient') {
            $patient_no = $opd_no;
            $patient_type = 'OPD';
        } elseif ($patient["patient_type"] == 'Inpatient') {
            $patient_no = $patient["ipd_no"];
            $patient_type = 'IPD';
        }
