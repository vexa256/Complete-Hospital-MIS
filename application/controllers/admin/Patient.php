...
class patient extends Admin_Controller {
    ...
    public function import() {
        ...
        $this->form_validation->set_rules('file', $this->lang->line('image'), 'callback_handle_csv_upload|required|trim|xss_clean');
    ...
    public function editTimeline() {
        ...
        if (!$this->rbac->hasPrivilege('edittimeline', 'can_view')) {
            access_denied();
        }
        $id = $this->input->post("id");
    ...
    public function add_prescription() {
        ...
        $this->form_validation->set_rules('medicine_cat[]', $this->lang->line('medicine'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $msg = array(
    ...
    public function ipdReport() {
        ...
        $additional_where = array("patients.is_active = 'yes'", "ipd_details.discharged != 'yes'");
        $doctorid = $this->input->post('doctor');

        if (!empty($doctorid)) {
            $additional_where = array("patients.is_active = 'yes' ", "ipd_details.cons_doctor ='" . $doctorid . "'", "ipd_details.discharged != 'yes'");
        }
        $disable_option = FALSE;
        $userdata = $this->customlib->getUserData();
        $role_id = $userdata['role_id'];
    ...
    public function updateBed() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array('id' => $this->input->post('patient_id'),
